...
| `REMOTE` | PC上の `decrypt_app` | Wi-Fi | 案A: PCへ配置 |
...
- **ウォームアップを必ず行う．** 初回はOpenSSLの遅延初期化・ページフォルト・
  TCPスロースタート・Wi-Fiの省電力復帰が乗り、外れ値になる．
...
- ラズパイとPCで **g++のバージョンと最適化フラグを揃える**こと．
  **x86のAES-NIとARMv8 Crypto Extensionsの有無**が「演算能力」差の主因になりうるため、
  レポートには必ず記載する．
- Wi-Fiは分散が大きい．平均だけでなく**中央値と最大値（テールレイテンシ）**を併記する．
  ロボット制御では最大値が支配的になる．
...
盗聴により鍵が漏洩する．競技会場の共用Wi-Fi環境ではこれは実運用上の欠点であり、
性能効率性とは別軸（セキュリティ）のデメリットとして評価に含めること．
