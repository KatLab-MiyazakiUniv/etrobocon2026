    ...
    CameraServer::ColorRegionDetectorResponse& response)
{
  ...
  Logger::printfLog(Logger::INFO, "[Perf] 色領域検出(detector.detect)時間: %d ms", t_detect_end - t_detect_start);
  ...
  MultiThread::wrap([=]() mutable {
    ...
    Logger::printfLog(Logger::INFO, "[Perf] 【別スレッド内】フレーム保存処理時間: %d ms", t_inner_save_end - t_inner_save_start);
  });
  int t_save_dispatch_end = ClockUtil::now();
  Logger::printfLog(Logger::INFO, "[Perf] フレーム保存非同期タスク発行時間: %d ms", t_save_dispatch_end - t_save_dispatch_start);

  int totalEndTime = ClockUtil::now();
  Logger::printfLog(Logger::INFO, "[Perf] execute全体処理時間: %d ms", totalEndTime - totalStartTime);
}
