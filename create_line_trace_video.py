...
def parse_args():
    parser = argparse.ArgumentParser(description="高速画像動画化ツール (Python + FFmpeg)")
...
def collect_and_sort_images(input_dir):
    """
    ディレクトリ内の 'roi_' で始まり、拡張子がJPEG/jpg/png等のファイルを収集し、
    ...
    if args.clean and success_count > 0:
        ...
        for path in processed_files:
            try:
                ...
                print(f"警告: ファイルの削除に失敗しました: {path} ({e})", file=sys.stderr)
        print(f"{deleted_count} 枚の入力画像を削除しました。")
