...
RouteFollower::RouteFollower(Robot& _robot, const EtRallyMap& _map, const MapData& _mapData,
                             double _targetSpeed, double _qrTrackingSpeed, int _qrTargetX,
...
void RouteFollower::straightWithQrCorrection(double distance)
{
   ...
   *   qrTrackingPid
   */
  CameraTracking cameraTracking(robot, std::move(condition), qrTrackingSpeed, qrTargetX,
                                qrTrackingPid, qrRequest, false);
...
bool RouteFollower::detectQrCode()
{
  ...
  const bool success = client.executeQrCodeDetection(request, response);

  if(!success) {
    Logger::warning("RouteFollower: QR detection communication failed");

    return false;
  }

  if(!response.wasDetected) {
    Logger::info("RouteFollower: QR code was not detected");

    return false;
  }

  Logger::info("RouteFollower: QR code detected");
...
void RouteFollower::runGateSegment(const RouteState& from, const RouteState& to, double distance)
{
   ...
   */
  const double beforeQrDistance = distanceToGate - QR_DETECTION_BEFORE_GATE;
  ...
  if(beforeQrDistance < 0.0) {
    Logger::warning("RouteFollower: QR detection point is behind start -> Straight");
  ...
  if(beforeQrDistance > 0.0) {
    Logger::printfLog(Logger::INFO, "RouteFollower: Straight before QR = %.2f mm",
                      beforeQrDistance);

    straight(beforeQrDistance);
  }
  ...
  Logger::info("RouteFollower: stop before QR detection");

  /*
   * Straight終了後に左右モーターを確実に停止する。
   */
  robot.getWheelMotorControllerInstance().stopBoth();

  /*
   * 停止直後は車体が完全に静止していない可能性があるため、
   * QRコードを安定して撮影できるまで待機する。
   */
  std::this_thread::sleep_for(std::chrono::milliseconds(QR_DETECTION_STOP_TIME));

  // =====================================================
  // 6. QRコードを確認
  // =====================================================

  Logger::info("RouteFollower: QR detection start");

  const bool qrDetected = detectQrCode();

  // =====================================================
  // 7. QR検出地点から走れる残り距離
  // =====================================================

  const double availableDistance = distance - beforeQrDistance;
   ...
   * 区間外へ出ないよう残り距離に制限する。
   */
  const double qrSectionDistance = std::min(QR_TRACKING_DISTANCE, availableDistance);
  ...
  if(qrDetected) {
    Logger::printfLog(Logger::INFO, "RouteFollower: QR detected -> CameraTracking %.2f mm",
                      qrSectionDistance);

    straightWithQrCorrection(qrSectionDistance);

  } else {
    Logger::printfLog(Logger::INFO, "RouteFollower: QR not detected -> Straight %.2f mm",
                      qrSectionDistance);

    straight(qrSectionDistance);
  }
  ...
  const double remainingDistance = distance - beforeQrDistance - qrSectionDistance;

  if(remainingDistance > 0.0) {
    Logger::printfLog(Logger::INFO, "RouteFollower: remaining Straight = %.2f mm",
                      remainingDistance);
