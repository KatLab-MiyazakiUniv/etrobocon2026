...
namespace {
   ...
   *
   * カメラバッファに古い画像が残っている可能性があるため、
   * 3回取得し、最後のフレームだけを検出に使用する。
   */
                                           ...
                                           CameraServer::SquareDetectorResponse& response)
{
  ...
  Logger::info("SquareDetectionActionHandler: "
               "3rd frame is used for detection");

  // =====================================================
  // 3. ROI設定
  // =====================================================

  const cv::Rect localRoi(request.roi.x, request.roi.y, request.roi.width, request.roi.height);
